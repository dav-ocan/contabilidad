...
## Bug encontrado y corregido: salto enorme de filas al registrar un gasto nuevo (2026-09-01)
Al probar el formulario "Registrar gasto" con un envio de prueba real, la fila nueva aparecio en `cuentas` en la fila 992 en vez de la 107 (justo despues de la ultima fila real). Causa raiz: `gspread`'s `append_row()` (usado en `guardar_nuevo_afiliado_en_google_sheets` y `upsert_salario_data`, ambos en `afiliacion_handler.py`) siempre agrega la fila en `row_count + 1`, donde `row_count` es la dimension del GRID de la hoja (una propiedad estructural de Sheets), no la posicion de la ultima fila con datos reales. `cuentas` tenia el grid dimensionado a 991 filas (con solo 106 con datos) y `Data` a 1000 filas (con solo 1, el encabezado) — probablemente de alguna prueba o resize manual antiguo que nunca se revirtio. Como limpiar el CONTENIDO de una fila no reduce la dimension del grid, el hueco se mantenia y crecia con cada envio.

**Corregido** ajustando la dimension del grid (`worksheet.resize(rows=N)`, via la API de Sheets, sin tocar ningun dato) para que quede exactamente igual a la ultima fila con datos reales:
- `cuentas`: de 991 a 106 (ajustado automaticamente a 107 tras la prueba, luego vuelto a 106 al borrar la fila de prueba).
- `Data`: de 1000 a 2 — el minimo que permite Sheets (no deja reducir a 1 sola fila; "no es posible borrar todas las filas no congeladas"). Como consecuencia, el PRIMER registro real que se guarde en `Data` va a caer en la fila 3 en vez de la 2 (deja la fila 2 vacia, un salto de 1 sola fila) — es una limitacion de Sheets, no del codigo. A partir de esa primera fila, cada envio nuevo cae inmediatamente despues del anterior sin hueco (verificado con 2 pruebas reales consecutivas en `cuentas`, borradas despues de confirmar).

Verificado en produccion real: se registraron 2 gastos de prueba via `curl`/`requests` contra el formulario Django corriendo localmente (apuntando al Sheet real) — el primero, antes del fix, cayo en la fila 992; tras redimensionar el grid, el segundo cayo exactamente en la fila 107. Ambas filas de prueba se borraron despues de confirmar.

**Pendiente de vigilar**: si en el futuro alguien inserta filas manualmente en el Sheet (ej. "Insertar 500 filas" desde el menu de Sheets) o pega un rango grande y lo borra despues, el grid puede volver a inflarse y reaparecer el salto. No hay forma de prevenir esto desde el codigo Python actual (gspread no expone un modo "buscar la ultima fila real" para `append_row`); si vuelve a pasar, la solucion es la misma: `worksheet.resize(rows=<ultima fila con datos>)`.

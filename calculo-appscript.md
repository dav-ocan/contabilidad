...
function syncCuentasToCalculos() {
  ...
  if (srcIdxPersona === -1) {
    throw new Error("Falta encabezado 'Persona' en 'Data'.");
  }
  ...
  // "Saldo inicial" y "Entrada banco" son un valor del MES (no por persona):
  // si aparecen en mas de una fila del mismo mes (varias personas, o
  // reenvios para corregir), se usa el valor MAS RECIENTE, nunca se suman.
  //
  ...
  dataRows.forEach(function(row, rowIndex) {
    ...
    if (saldoInicialAmount) {
      if (totals.saldoInicial && totals.saldoInicial !== saldoInicialAmount) {
        Logger.log("Aviso: 'Saldo inicial' de " + monthKey + " cambio de " + totals.saldoInicial + " a " + saldoInicialAmount + " (se uso el valor mas reciente).");
      }
    ...
    if (entradaBancoAmount) {
      if (totals.entradaBanco && totals.entradaBanco !== entradaBancoAmount) {
        Logger.log("Aviso: 'Entrada banco' de " + monthKey + " cambio de " + totals.entradaBanco + " a " + entradaBancoAmount + " (se uso el valor mas reciente).");
      }
      totals.entradaBanco = entradaBancoAmount;
    }

    var personaKey = normalizeHeader_(row[srcIdxPersona]);
    var mapKey = personaKey ? (personaKey + "|" + monthKey) : ("__fila" + rowIndex);
  ...
  Object.keys(latestByPersonaMonth).forEach(function(mapKey) {
    ...
    var monthKey = entryData.monthKey;
    var totals = getMonthTotals_(monthTotals, monthKey);
    ...
    if (cobros.length) {
      cobros.forEach(function(cobro) {
        ...
        dataByMonth[monthKey].push({
          saldoInicial: entryData.saldoInicial,
          entradaBanco: entryData.entradaBanco,
